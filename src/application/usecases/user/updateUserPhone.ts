...
import { validateChangePhoneNumber } from '../../../utils/joi/schemas/userSchema.joi';
...
import { userPhoneChanged } from '../../../utils/kafka-topics.json';

export default class UpdateUserPhone
  implements
    UseCaseInterface<IUpdateUserPhoneDTO, IReturnValue<UserEntity | null>>
{
  constructor(
  ...
  async execute(
    ...
    try {
      ...
      // Verify otp code to ensure user is still in valid session to update phone number
      const verifiedToken = foundUser.tokens?.find(
        (item) => item?.token === params.otpCode
      );

      // If token is not found, then user has lost his ability to change phone number. User must reauthenticate for a new phone number
      if (!verifiedToken) {
        return {
          success: false,
